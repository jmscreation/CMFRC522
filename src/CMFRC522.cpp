...
MFRC522::MFRC522(	byte chipSelectPin,		///< Arduino pin connected to MFRC522's SPI slave select input (Pin 24, NSS, active low)
	...
	if(_resetPowerDownPin != UNUSED_PIN){
		gpioSetMode(_resetPowerDownPin, PI_OUTPUT);
	}
	if(_chipSelectPin != UNUSED_PIN){
...
void MFRC522::initSpi() {
    if(!SPI::init(MFRC522_SPICLOCK))
		throw std::runtime_error("Spi initialization failed...");
    std::cout << "SPI initialization succeeded...\n";
}
...
void MFRC522::PCD_WriteRegister(	PCD_Register reg,	///< The register to write to. One of the PCD_Register enums.
	...
	// SPI.beginTransaction(SPISettings(MFRC522_SPICLOCK, MSBFIRST, SPI_MODE0));	// Set the settings to work with SPI bus
	digitalWrite(_chipSelectPin, PI_LOW);		// Select slave
	SPI::transfer(reg);						// MSB == 0 is for writing. LSB is not used in address. Datasheet section 8.1.2.3.
	for (byte index = 0; index < count; index++) {
		SPI::transfer(values[index]);
	}
	digitalWrite(_chipSelectPin, PI_HIGH);		// Release slave again
	// SPI.endTransaction(); // Stop using the SPI bus
} // End PCD_WriteRegister()
...
void MFRC522::PCD_Init() {
	...
	if (_resetPowerDownPin != UNUSED_PIN) {
		digitalWrite(_resetPowerDownPin, PI_LOW);		// Make sure we have a clean LOW state.
		delay(1);	// 8.8.1 Reset timing requirements says about 100ns. Let us be generous: 1ms
		digitalWrite(_resetPowerDownPin, PI_HIGH);		// Exit power down mode. This triggers a hard reset.
 ...
 */
bool MFRC522::PCD_PerformSelfTest(bool log) {
	...
	for (uint8_t i = 0; i < 64; i++) {
		if(log){
			std::cout << std::hex << int(reference[i]) << " : " << int(result[i]) << "\n";
		}
